...
export async function POST(request: NextRequest) {
  try {
    ...
    return new NextResponse(
      ...
      <html>
        <head>
          <title>Apple Health Connected</title>
        </head>
        <body>
          <script>
            // Signal to parent window that connection was successful
            if (window.opener) {
              window.opener.postMessage({ type: 'apple-health-connected' }, '*');
            }
          ...
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; text-align: center; padding: 40px;">
            ...
            <p>Apple Health has been connected successfully.</p>
      ...
      <html>
        ...
        <body>
          <script>
            ...
            if (window.opener) {
              window.opener.postMessage({ type: 'apple-health-error' }, '*');
            }
            setTimeout(() => {
              window.close();
            }, 3000);
          ...
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; text-align: center; padding: 40px;">
            ...
            <p>Failed to connect Apple Health. Please try again.</p>
            <p style="font-size: 14px; color: #666;">This window will close automatically...</p>
          </div>
        </body>
      </html>`,
      ...
      <html>
        <body>
          <div style="font-family: sans-serif; text-align: center; padding: 40px;">
            <h1>Apple Health OAuth Callback</h1>
            <p>This endpoint handles Apple Health authentication callbacks.</p>
          </div>
        </body>
      </html>`,
      {
